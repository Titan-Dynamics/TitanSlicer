...
static void start_new_slicer_or_gcodeviewer(const NewSlicerInstanceType instance_type, const std::vector<wxString> paths_to_open, bool single_instance)
{
	...
	path += "\\";
	path += (instance_type == NewSlicerInstanceType::Slicer) ? "titan-slicer.exe" : "bambu-gcodeviewer.exe";
...
#else // Linux or Unix
	{
		...
		if (args.empty()) {
			...
			my_path = (bin_path.parent_path() / ((instance_type == NewSlicerInstanceType::Slicer) ? "titan-slicer" : "bambu-gcodeviewer")).string();
