...
bool DesktopIntegrationDialog::is_integrated()
{
    ...
    // confirmation that TitanSlicer.desktop exists
    struct stat buffer;   
...
void DesktopIntegrationDialog::perform_desktop_integration()
{
    ...
    for (size_t i = 0; i < target_candidates.size(); ++i) {
        // Copy icon TitanSlicer.png from resources_dir()/icons to target_dir_icons/icons/
        if (contains_path_dir(target_candidates[i], "images")) {
            target_dir_icons = target_candidates[i];
            std::string icon_path = GUI::format("%1%/images/TitanSlicer.png",resources_dir());
            std::string dest_path = GUI::format("%1%/images/%2%TitanSlicer%3%.png", target_dir_icons, icon_theme_path, version_suffix);
    ...
    if(target_dir_icons.empty()) {
        BOOST_LOG_TRIVIAL(error) << "Copying TitanSlicer icon to icons directory failed.";
    } else 
    	// save path to icon
        app_config->set("desktop_integration_icon_slicer_path", GUI::format("%1%/images/%2%TitanSlicer%3%.png", target_dir_icons, icon_theme_path, version_suffix));
    ...
    for (size_t i = 0; i < target_candidates.size(); ++i)
    {
        if (contains_path_dir(target_candidates[i], "applications")) {
            ...
            std::string desktop_file = GUI::format(
                ...
                "StartupNotify=false\n"
                "StartupWMClass=titan-slicer\n", name_suffix, version_suffix, excutable_path);

            std::string path = GUI::format("%1%/applications/TitanSlicer%2%.desktop", target_dir_desktop, version_suffix);
            if (create_desktop_file(path, desktop_file)){
                BOOST_LOG_TRIVIAL(debug) << "TitanSlicer.desktop file installation success.";
                break;
            } else {
            	// write failed - try another path
                BOOST_LOG_TRIVIAL(debug) << "Attempt to TitanSlicer.desktop file installation failed. failed path: " << target_candidates[i];
    ...
    app_config->set("desktop_integration_app_path", GUI::format("%1%/applications/TitanSlicer%2%.desktop", target_dir_desktop, version_suffix));
    ...
    if (platform_flavor() != PlatformFlavor::LinuxOnChromium) {
        ...
        if (!target_dir_icons.empty())
        {
            std::string icon_path = GUI::format("%1%/images/TitanSlicer-gcodeviewer_192px.png",resources_dir());
            std::string dest_path = GUI::format("%1%/images/%2%TitanSlicer-gcodeviewer%3%.png", target_dir_icons, icon_theme_path, version_suffix);
            if (copy_icon(icon_path, dest_path))
                // save path to icon
        ...
        std::string desktop_file = GUI::format(
            ...
            "GenericName=3D Printing Software\n"
            "Icon=TitanSlicer-gcodeviewer%2%\n"
        ...
        std::string desktop_path = GUI::format("%1%/applications/TitanSlicerGcodeViewer%2%.desktop", target_dir_desktop, version_suffix);
        if (create_desktop_file(desktop_path, desktop_file))
            ...
            show_error(nullptr, _L("Performing desktop integration failed - could not create Gcodeviewer desktop file. TitanSlicer desktop file was probably created successfully."));
        }
    }
...
void DesktopIntegrationDialog::perform_downloader_desktop_integration(std::string url_prefix)
{
    ...
    std::string desktop_file_downloader = GUI::format(
        "[Desktop Entry]\n"
        "Name=TitanSlicer URL Protocol %1% %2%\n"
        "Exec=%3% %%u\n"
        "Terminal=false\n"
        "Type=Application\n"
    ...
    for (size_t i = 0; i < target_candidates.size(); ++i) {
        if (contains_path_dir(target_candidates[i], "applications")) {
            target_dir_desktop = target_candidates[i];
            // Write slicer desktop file
            std::string path = GUI::format("%1%/applications/TitanSlicerURLProtocol-%2%%3%.desktop", target_dir_desktop, url_prefix, version_suffix);
            if (create_desktop_file(path, desktop_file_downloader)) {
                app_config->set("desktop_integration_URL_path", path);
                candidate_found = true;
                BOOST_LOG_TRIVIAL(debug) << "TitanSlicerURLProtocol.desktop file installation success.";
                break;
            }
            else {
                // write failed - try another path
                BOOST_LOG_TRIVIAL(debug) << "Attempt to TitanSlicerURLProtocol.desktop file installation failed. failed path: " << target_candidates[i];
                target_dir_desktop.clear();
            }
        }
    }
    ...
    std::string desktop_path = GUI::format("%1%/applications/TitanSlicerURLProtocol-%2%%3%.desktop", target_dir_desktop, url_prefix, version_suffix);
    if (create_desktop_file(desktop_path, desktop_file_downloader)) {
        ...
        std::string command = GUI::format("xdg-mime default TitanSlicerURLProtocol-%1%%2%.desktop x-scheme-handler/%1%", url_prefix, version_suffix);
